...
class UserService extends BaseService {
  ...
  Stream<UserData> singleUser(String? id, {String? searchText}) {
    if (id.validate().isEmpty) {
      return Stream.value(UserData());
    }
    if (id.validate().isDigit()) {
      return ref!.where('id', isEqualTo: id.toInt()).limit(1).snapshots().map((event) {
        if (event.docs.isEmpty) {
          // User document not found by numeric ID — return fallback
          return UserData()..id = id.toInt();
        }
      ...
      return ref!.where('uid', isEqualTo: id).limit(1).snapshots().map((event) {
        if (event.docs.isEmpty) {
          // User document not found by Firebase UID — return fallback with uid set
