...
class ChatItemWidget extends StatefulWidget {
  ...
  ChatItemWidget({required this.chatItemData, this.bookingId});
...
class _ChatItemWidgetState extends State<ChatItemWidget> {
  ...
  void deleteMessage() async {
    ...
    if (res ?? false) {
      ...
      chatServices.deleteSingleMessage(receiverId: widget.chatItemData.receiverId!, documentId: widget.chatItemData.uid.validate(), bookingId: widget.bookingId).then((value) {
  ...
  Widget build(BuildContext context) {
    // Determine message time safely — Zeerah messages use Timestamp ('createdAtTime'),
    // provider messages also populate 'createdAt' (epoch millis).
    DateTime messageDate;
    String currentDateTime;
    if (widget.chatItemData.createdAtTime != null) {
      messageDate = widget.chatItemData.createdAtTime!.toDate();
      currentDateTime = messageDate.toString();
    } else if (widget.chatItemData.createdAt != null) {
      messageDate = DateTime.fromMicrosecondsSinceEpoch(widget.chatItemData.createdAt! * 1000);
      currentDateTime = widget.chatItemData.createdAt.validate().toString();
    } else {
    ...
    if (messageDate.day == now.day && messageDate.month == now.month && messageDate.year == now.year) {
      time = formatDate(currentDateTime, format: DATE_FORMAT_3, isFromMicrosecondsSinceEpoch: false, isTime: true);
      ...
      time = formatDate(currentDateTime, format: DATE_FORMAT_1, isFromMicrosecondsSinceEpoch: false);
    }

    Widget chatItem(String? messageTypes) {
      // Zeerah user messages have no messageType — default to treating them as text.
      final bool isText = messageTypes == null || messageTypes.isEmpty ||
          messageTypes == MessageType.TEXT.name || messageTypes == 'TEXT';
      return isText
          ? Column(
              ...
              children: [
                Text(
                  widget.chatItemData.message.validate(),
