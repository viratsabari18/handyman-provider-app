...
class _UserChatScreenState extends State<UserChatScreen> with WidgetsBindingObserver {
  ...
  // FIXED: roomId generation consistent with chat_services.dart
  String get roomId => 'booking_${widget.bookingId}_${widget.targetChatId}';

  // Stream subscription to mark read when new messages arrive while screen is open
...
void init() async {
  ...
  log("Chat Room ID => $roomId");
  log("================================");

  // Mark all messages as read when screen opens
  await _markAllMessagesAsRead();

  // Listen to new messages and mark read immediately
  _messagesStream = chatServices.chatMessagesWithPagination(roomId: roomId).snapshots();
  ...
  // Helper to call the mark-read service
    ...
    return Row(
      children: [
        AppTextField(
          ...
          onFieldSubmitted: (s) => sendMessages(),
  ...
  // =========================================================
  // SEND MESSAGE (same as before, but we keep as is)
  // =========================================================
  Future<void> sendMessages() async {
    if (appStore.isLoading) return;
    String text = messageCont.text.trim();
    if (text.isEmpty) {
    ...
    // Block phone numbers
    final String cleanedText = text.replaceAll(RegExp(r'[^0-9]'), '');
    ...
    List<String> blockedWords = [
      'sex', 'nude', 'naked', 'porn', 'boobs', 'breast', 'xxx', 'adult',
      'fuck', 'bitch', 'motherfucker', 'asshole', 'dick', 'pussy', 'bastard', 'shit'
    ];
    String lowerText = text.toLowerCase();
    bool hasBadWord = blockedWords.any((word) => lowerText.contains(word));
    if (hasBadWord) {
...
final Map<String, dynamic> data = {
  ...
  'attachmentfiles': [],
    ...
    await chatServices.addMessage(roomId: roomId, data: data);
    messageCont.clear();
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    // If the app returns to the foreground, mark messages as read again
              ...
              if (index == 0) {
                showConfirmDialogCustom(
                  ...
                  onAccept: (c) async {
                    ...
                    await chatServices.clearAllMessages(roomId: roomId)
                        .then((value) {
            ...
            children: [
              Container(
                ...
                child: FirestorePagination(
                  ...
                  query: chatServices.chatMessagesWithPagination(roomId: roomId),
